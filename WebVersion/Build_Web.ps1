...
New-Item -ItemType Directory -Force (Join-Path $PSScriptRoot 'site'), (Join-Path $PSScriptRoot 'reports') | Out-Null
if (-not (Test-Path -LiteralPath (Join-Path $PSScriptRoot 'toolchain\web_nothreads_release.zip'))) { throw 'Web export template missing. Run: python WebVersion/tools/fetch_web_templates.py' }
